...
- ⌨️ **快捷键**: 支持 Ctrl+B 快捷键
- 📱 **移动端优化**: 自定义虚拟键盘，包含方向键、Tab、Ctrl+C、Enter、滚动模式等快捷键，支持长按连发
...
**新功能**
- ✨ 新增终端选择模式，支持复制终端内容（桌面端和移动端均可使用）
- ✨ 虚拟键盘新增 Shift+Tab 按钮，用于切换 Claude Code 编码模式
...
**复制终端内容**
- 点击工具栏上的 **复制图标** 进入选择模式
- 在弹出的窗口中，**点击行**选择要复制的内容
  - 桌面端：**Shift+点击**可快速选择范围
  - 移动端：逐行点击选择多行
- 点击 **"全选"** 按钮选择所有内容
- 点击 **"复制选中"** 按钮复制选中的内容
- 点击 **"关闭"** 退出选择模式
...
1. **实时模式**（默认）：语音识别结果实时发送到终端，类似手动输入字符
2. **命令模式**：语音识别结果填入命令输入框，修改后再发送

**使用方法：**

1. 点击输入框旁边的 **⚡ 闪电按钮** 切换模式：
   - 蓝色高亮 = 实时模式
   - 灰色 = 命令模式

2. **语音输入方式**：
   - **长按** 🎤 麦克风按钮开始语音输入
   - 说话完成后 **松开** 按钮，识别结果会自动处理
   - 最大录音时长 30 秒

3. **命令模式使用流程**（推荐）：
   - 确认闪电按钮为灰色（命令模式）
   - 长按 🎤 麦克风按钮开始语音输入
   - 说话完成后松开按钮，识别结果会自动填入命令输入框
   - 输入框会自动获得焦点，方便你修改识别不准确的文字
   - 确认无误后点击 "发送" 按钮或按 Enter 发送

**适用场景：**
- 命令模式适合需要修改识别结果后再发送的场景
- 实时模式适合快速输入简单命令
